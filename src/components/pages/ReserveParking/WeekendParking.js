...
const WeekendParking = ({ index, touched, }) => {
  const sigPad = useRef(null);
  // const { fields, append, remove } = useFieldArray({
  //   control,
  //   name: "vehicleDetails", // Make sure to match your form field name
  // })
  ...
  const handleVehicleCountChange = (e) => {
    ...
    // Update the vehicle number state
    // Amountdetails(e); // Fetch vehicle data based on the input
  };
  ...
  const handleClearSignature = () => {
    if (sigPad.current) {
      ...
      setSignatureDataUrl('');
      setSignatureError(''); // Clear error on clear
    }
  };
  ...
  const submitForm = async (event) => {
    ...
    try {
      ...
      Swal.fire({
        position: 'top-end',
        icon: 'success',
        title: 'Weekend parking enrolled successfully.',
        showConfirmButton: true,
        confirmButtonText: 'OK',
      });
  ...
  const handleStartDateChange = (date) => {
    ...
    if (date) {
      ...
      calculatedEndDate.setDate(calculatedEndDate.getDate() + 3); // Set end date to 3 days after start date
      setEndDate(calculatedEndDate); // Automatically set the end date
      calculateTotalDays(date, calculatedEndDate); // Calculate total days
    }
  };
  const handleEndDateChange = (date) => {
    setEndDate(date);
    if (startDate && date) {
      calculateTotalDays(startDate, date); // Calculate total days based on selected start and end date
    }
  };
  const isFriday = (date) => {
  ...
  const isValidEndDate = (date) => {
    if (startDate) {
      ...
      minDate.setDate(minDate.getDate() + 1); // End date must be at least 1 day after the start date
      maxDate.setDate(maxDate.getDate() + 3); // End date can be up to 3 days after the start date
  ...
  const DaysList = async (startDate, endDate) => {
    ...
    let parkingTypeObjectId = '66a2397aafd7ace00b1d43db';
  ...
  const handleFocus = async (latitude, longitude) => {
    console.log("handleFocus messiiiiiiiiiiiiii");
    // const lists = {
    //   latitude: 33.797112,
    //   longitude: -84.422771,
    // };
    try {
      ...
      console.log(res, 'suiiiiii-weekend');
      if (res?.data?.success) {
  ...
  const Vehicletype = async (vehicle) => {
    try {
      let parkingTypeId = '66a2397aafd7ace00b1d43db';
  ...
  const Amountdetails = async () => {
    ...
    try {
      ...
      Swal.fire({
        position: 'top-end',
        icon: 'error',
        title: err.response?.data?.message || 'An error occurred ccccatch',
        showConfirmButton: true,
        confirmButtonText: 'OK',
      });
      console.log('error', err);
  ...
  return (
    ...
    <>
      ...
      <div>
        ...
        open={loadingmodule} // Make sure open is tied to loadingmodule
      > */}
            ...
            <div className="row pb-4">
              <div className="col-md-12">
                ...
                <Accordion>
                  ...
                  >
                    <Typography>
                      WEEKEND PARKING AGREEMENT - TERMS AND CONDITIONS (CLICK TO
                      EXPAND)
                    </Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                  ...
                  {({ values, errors, touched }) => (
                    <Form id="quotation" noValidate>
                      ...
                      <div className="step">
                        ...
                        <h5>On the days listed below: <b>(The Start Date Must be Friday; End Date Must be Monday)</b></h5>
                        <div className="row">
                          <div className="col-md-4 col-sm-4">
                            <div className="form-group">
                              ...
                              <DatePicker
                                ...
                                filterDate={isFriday} // Only allow Fridays to be selected
                              />
                              {firstdaterror && (
                                <div className="invalid-feedback">
                                  {firstdaterror}
                          ...
                          <div className="col-md-4 col-sm-4">
                            <div className="form-group">
                              ...
                              <DatePicker
                                ...
                                filterDate={isValidEndDate} // Only allow valid end dates
                                minDate={startDate} // Ensure end date is after or equal to start date
                              />
                              {lastdateerror && (
                                <div className="invalid-feedback">
                                  {lastdateerror}
                              ...
                              <>
                                {values.vehicles.map((vehicle, index,) => (
                                  ...
                                  <div key={index} className="row vehicle-details">
                                    {/* Number of Vehicles */}
                                    <div className="col-md-6 col-sm-6" style={{ left: '90px ' }}>
                                      <div className="form-group">
                                        <label>
                            ...
                            <div className="col-md-12 mb-3">
                              <Accordion>
                                ...
                                <AccordionDetails>
                                  ...
                                  <FormControlLabel
                                    required
                                    control={<Switch checked={isAcknowledged.vehicleRelocation} onChange={handleSwitchChange('vehicleRelocation')} />}
                                    label="Required"
                                  />
                            ...
                            {showExtendedParkingInfo && (
                              <div className="col-md-12">
                                ...
                                <div className="form-group">
                                  ...
                                  />
                                  <button className='button' type="button" onClick={handleClearSignature}>
                                    Clear
                                  </button>
                                  {signatureDataUrl && (
                                    <div>
